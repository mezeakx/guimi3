...
所有 AI 模型调用必须通过 DeepSeek API，禁止接入任何其他 AI 服务商（OpenAI / Claude / 其他）。
...
3. **API 调用失败** — 传递给 DeepSeek API 时返回 `BadRequestError: Expecting property name enclosed in double quotes`
